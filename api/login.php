...
if (isset($_GET['destroy'])) {
	...
	$res = $dbhelper->query('SELECT id,username, `type` FROM users WHERE username=:username AND password=:password', array(':username' => $_POST['username'], ':password' => $_POST['password']))[0] ?? null;
	if (empty($res)) {
		echo json_encode(array('error' => 'Username and password not match!'));
	} else {
		session_start();
