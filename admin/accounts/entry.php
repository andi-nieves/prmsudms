...
$rooms = $dbhelper->query("SELECT `room_list`.id, CONCAT((SELECT `dorm_list`.name FROM `dorm_list` WHERE `dorm_list`.id = `room_list`.dorm_id AND `dorm_list`.delete_flag=0)) as name, `room_list`.slots - (SELECT COUNT(`account_list`.id) FROM `account_list` WHERE `account_list`.room_id = `room_list`.id AND `account_list`.delete_flag=0) as slots, `room_list`.price, `room_list`.delete_flag FROM `room_list` WHERE `room_list`.delete_flag=0 and status=1;");
